"""
test_latency.py — 測量 POST /api/v1/detect/image 的分段 latency
====================================================================
...
用法：
    python test_latency.py --url http://<server-ip>:8080 --image sample_images/xxx.jpg
    python test_latency.py --url http://127.0.0.1:8080 --image sample_images/xxx.jpg --rounds 10

參數：
    --url          server base url（不含 path），例如 http://192.168.1.10:8080
    --image        要上傳測試的圖片路徑
    --rounds       /detect/image 測試次數（預設 5）
...
def measure_detect_image(base_url: str, image_path: Path, offset: float) -> dict:
    """打一次 /api/v1/detect/image，回傳拆分後的三段 latency（單位：秒）。"""
...
def main():
    parser = argparse.ArgumentParser(description="測量 /api/v1/detect/image 的分段 latency")
    ...
    parser.add_argument("--rounds", type=int, default=5, help="/detect/image 測試次數（預設 5）")
